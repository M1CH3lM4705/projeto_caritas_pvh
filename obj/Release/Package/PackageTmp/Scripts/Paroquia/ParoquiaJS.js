...
$(document).on('click', '.btnAlterarParoquia', function () {
    ...
    bootbox.confirm({
        message: "Atenção! Ao excluir a paroquia, todos os agentes caritas relacionados a está paroquia serão perdidos. \nTem certeza que deseja excluir essa Paróquia?",
        buttons: {
            confirm: {
                label: 'Sim',
...
$(document).on('change', '#UF', function () {
    ...
    var municipios = $("#Municipio_id");
        ...
        $.getJSON("https://servicodados.ibge.gov.br/api/v1/localidades/estados/"+ estados +"/municipios", function (data) {
            $(data).each(function () {
                $("<option value=" + this.id + ">" + this.nome + "</option>").appendTo(municipios);
            });
        });
