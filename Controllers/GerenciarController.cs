...
namespace ProjetoBetaAutenticacao.Controllers
{
    public class GerenciarController : Controller
    {
        ...
        public GerenciarController(ApplicationUserManager userManager, ApplicationSignInManager signInManager)
        {
        ...
        {
            if(!Utils.ValidarSenhaAtual(User.Identity.GetUserId(), model.OldPassword))
            {
                ModelState.AddModelError("", "A senha atual nao confere.");
            }
            if (!ModelState.IsValid)
            {
                return View(model);
        ...
        public ActionResult LinkLogin(string provider)
        {
            ...
            return new AutenticacaoController.ChallengeResult(provider, Url.Action("LinkLoginCallback", "Manage"), User.Identity.GetUserId());
        }
